  ...
  class ABCTomM8 {
    ...
    function generate_datatable($table_name, $fields_array, $primary_key_name, $where_clause, $order_array = array(), $limit_clause = "", $page_name, $display_show = true, $display_edit = true, $display_delete = true, $sortable_columns = false, $paginate_table = false, $date_format = "Y-m-d", $filter_arrays = array()) {
      ...
      if (!is_array($fields_array)) {
        ...
        if (count($filter_arrays) > 0) {
          if (ABCTomM8::get_query_string_value($table_name."_filters") != "") {
            $filters = explode(",", ABCTomM8::get_query_string_value($table_name."_filters"));
            foreach ($filters as $filter) {
              if (ABCTomM8::get_query_string_value("filter_".$filter) != "") {
                if (!(isset($_POST["action"]) && $_POST["action"] == "Reset")) {
                  array_push($extra_where, $filter." LIKE '%".ABCTomM8::get_query_string_value("filter_".$filter)."%'");  
                }
              }
            }
        ...
        $results = ABCTomM8::get_results($table_name, $fields_array, $where_clause, $order_array, $limit_clause.$offset_clause);
        
        $total_count = count(ABCTomM8::get_results($table_name, $fields_array, $where_clause));
              ...
              <fieldset>
                ...
                foreach($filter_arrays as $filter_array) { 
                  foreach($filter_array as $key => $value) { 
                    if (ABCTomM8::get_query_string_value("filter_".$key) != "") {
                      if (isset($_POST["action"]) && $_POST["action"] == "Reset") {
                        $_POST["filter_".$key] = "";
                      }
                      $params_filter .= "&filter_".$key."=".ABCTomM8::get_query_string_value("filter_".$key);
                    }
                    array_push($filters, $key);
                    ABCTomM8::add_form_field(null, $value["type"], titlize_str($key), "filter_".$key, "filter_".$key, array(), "p", array(), $value["value_options"]);
                  }
                } 
                ?>
        ...
        if (count($results) > 0) { 
          if ($paginate_table) { ABCTomM8::generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "top"); } ?>
            <table class="data">
              <thead>
                <tr>
                  <?php foreach($fields_array as $field_name) { ?>
                    <th class='<?php echo(strtolower(str_replace("_", "-", $field_name))); ?>'>
                      ...
                      <?php } ?>
                      <?php echo(ABCTomM8::titlize_str($field_name)); ?>
                      <?php if ($sortable_columns) {
                  ...
                  <tr>
                    <?php foreach($fields_array as $field_name) { ?>
                      <td class='<?php echo(esc_html(strtolower(str_replace("_", "-", $field_name)))); ?>'>
                        <?php 
                          if (ABCTomM8::is_valid_datetime($result->$field_name)) {
            ...
            <?php if ($paginate_table) { ABCTomM8::generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, "bottom"); } ?>
        <?php } else {
    ...
    function generate_datatable_pagination($table_name, $total_count, $limit_clause, $page_no, $page_name, $order_direction, $pagination_class) {
      ...
      $total_number_pages = intval($total_count / $limit_clause);
        $params_filter = "";
        $filters = explode(",", ABCTomM8::get_query_string_value($table_name."_filters"));
        $params_filter .= "&".$table_name."_filters=".ABCTomM8::get_query_string_value($table_name."_filters");
        foreach ($filters as $filter) {
          if (ABCTomM8::get_query_string_value("filter_".$filter) != "") {
            $params_filter .= "&filter_".$filter."=".ABCTomM8::get_query_string_value("filter_".$filter);
          }
        }
    ...
    function generate_displayview($table_name, $fields_array, $id_column_name, $id) {
      if (!is_array($fields_array)) {
        ...
        $result = ABCTomM8::get_row_by_id($table_name, $fields_array, $id_column_name, $id);
        echo("<dl>");
    ...
    function validate_value($validation, $value, $error_session_name) {
      ...
      if ($value != "") {
        ...
        if (preg_match("/date | date|^date$/i", $validation)) {
          if (!ABCTomM8::is_valid_datetime($value)) {
        ...
        if (preg_match("/email | email|^email$/i", $validation)) {
          if (!ABCTomM8::is_valid_email($value)) {
        ...
        if (preg_match("/multi-emails | multi-emails|^multi-emails$/i", $validation)) {
          if (!ABCTomM8::is_valid_emails($value)) {
    ...
    function validate_form($validations_array) {
      ...
      foreach ($validations_array as $key => $value) {
        if (is_array(ABCTomM8::get_query_string_value($key))) {
          $index = 0;
          foreach (ABCTomM8::get_query_string_value($key) as $sub_value) {
            if (ABCTomM8::validate_value($value, $sub_value, $key."_".$index."_error") == false) {
              $validate_form = false;
            }
            $index++;
          }
          ...
          if (preg_match("/required/i", $value) && isset($_POST[$key."_0"])) {
            ...
            if (is_array(ABCTomM8::get_query_string_value($key."_0"))) {
              // For checkbox fields.
              $index = 0;
              foreach ($_POST["validation_0"] as $row) {
                ...
                if (ABCTomM8::validate_value($value, $data, $key."_".$index."_error") == false) {
                  echo $index;
                  $validate_form = false;
                }
                $index++;
              ...
              if (ABCTomM8::validate_value($value, $data, $key."_error") == false) {
                $validate_form = false;
              }
            }
          } else {
            if (ABCTomM8::validate_value($value, ABCTomM8::get_query_string_value($key), $key."_error") == false) {
              $validate_form = false;
            }
          }
        }
    ...
    function add_form_field($instance, $field_type, $field_label, $field_id, $field_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      if ($instance == null && preg_match("/^tomm8te_admin_option::/", $field_name)) {
        ...
        if (count($_POST) > 0) {
          if ($field_index >= 0) {
            $field_value = ABCTomM8::get_query_string_value($field_name, $field_index);
          } else {
            $field_value = ABCTomM8::get_query_string_value($field_name);
          }
        }
      } else {
      ...
      if ($field_type == "text") {
        ...
        if (count($value_options) == 1) {
          ...
          foreach($value_options as $key => $option) {
            ...
            $field_value = ABCTomM8::get_query_string_value($field_name."_".$i, $field_index);
            $field_value = str_replace("&amp;", "&", htmlentities(htmlentities($field_value, ENT_NOQUOTES), ENT_QUOTES));
            $checked_value = "";
            if (count($_POST) == 0) {
    ...
    function add_option_form_field($field_type, $field_label, $field_id, $option_name, $field_attributes = array(), $container_element, $container_attributes = array(), $value_options = array(), $field_index = -1) {
      ...
      ABCTomM8::add_form_field(null, $field_type, $field_label, $field_id, "tomm8te_admin_option::".$option_name, $field_attributes, $container_element, $container_attributes, $value_options, $field_index);
    }
